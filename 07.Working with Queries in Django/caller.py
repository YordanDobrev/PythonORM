...
def find_books_by_genre_and_language(genre, language):
    book = Book.objects.filter(genre=genre, language=language)
    return book


def find_authors_nationalities():
    result = []
    author = Author.objects.filter(nationality__isnull=False)
...
def filter_authors_by_nationalities(national):
    result = []
    authors = Author.objects.filter(nationality=national).order_by("first_name", "last_name")
